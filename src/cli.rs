...
fn scan_subcommand() -> Command {
    Command::new("scan")
        ...
        .arg(Arg::new("malformed-verb-check").long("malformed-verb-check").help("Send malformed HTTP request methods (wrong case, embedded space/tab, control/null chars, non-token chars, overlong) over a raw socket to detect lax method parsing and method-ACL bypass. Off by default.").action(ArgAction::SetTrue))
