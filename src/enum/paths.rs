...
use crate::storage::sqlite::output_enum_paths_sqlite;
...
pub async fn run(matches: &ArgMatches) -> Result<()> {
    ...
    let extensions: Vec<String> = matches.get_one::<String>("extensions")
    ...
    let wildcard_probe = if wildcard_enabled {
        let url = format!("{}/{}", base_url.trim_end_matches('/'), "terminus-random-probe-xyz");
    ...
    queue.push_back(base_url.trim_end_matches('/').to_string());

    let semaphore = Arc::new(Semaphore::new(threads));
    let results = Arc::new(tokio::sync::Mutex::new(Vec::new()));

    let max_depth = if recursive { 1 } else { 0 };
    let mut depth = 0;

    while depth <= max_depth {
        let mut handles = Vec::new();
        let base = match queue.pop_front() {
            Some(b) => b,
            None => break,
        };

        for word in &words {
            let mut paths = vec![format!("{}/{}", base, word)];
            for ext in &extensions {
                paths.push(format!("{}/{}.{}", base, word, ext.trim_start_matches('.')));
            }
            ...
            for path in paths {
                ...
                let handle = tokio::spawn(async move {
                    let _permit = permit;
                    let (status, length) = match probe(&transport, &path).await {
                        Ok(val) => val,
                        Err(_) => return,
                    };
        ...
        if recursive {
            ...
            for row in guard.iter() {
                if row.status >= 200 && row.status < 400 && row.url.ends_with('/') {
                    queue.push_back(row.url.trim_end_matches('/').to_string());
                }
            }
        }
...
fn output_results(results: &[EnumResult], format: OutputFormat, output_base: Option<&str>) -> Result<()> {
    match format {
        ...
        OutputFormat::Sqlite => {
            output_enum_paths_sqlite(results, output_base)?;
        }
        OutputFormat::All => {
